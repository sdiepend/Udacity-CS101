...
#my solution, weird thought process I know.
def hexes_to_udaciousness(n, spread, target):
    hexes = 1
    if n >= target:
        return hexes - 1 
    else:
        return hexes + hexes_to_udaciousness(n+(n*spread), spread, target)

#Solution from the videos, 
def hexes_to_udaciousness2(n, spread, target):
    if n >= target:
        return 0 
    else:
