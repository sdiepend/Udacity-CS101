...
# My solution, could use some improvements/cleaning at this point. Have some 
# overhead in the ordered_search function. 
def ordered_search(index, ranks, keyword):
    if keyword not in index:
        return None
    else:
        results_uo = lookup(index, keyword)
        unordered_ranks = []
        for url in ranks:
            for link in results_uo:
                if link == url:
                    unordered_ranks.append([link, ranks[link]])
        

        ordered_ranks = quicksort(unordered_ranks)

        cleaned_ranks = []
        i = 0
        while i < len(ordered_ranks):
            cleaned_ranks.append(ordered_ranks[i][0])
            i = i + 1
...
def quicksort(to_sort):
    if len(to_sort) == 0:
        ...
        i = 1
        while i < len(to_sort):
            if to_sort[i][1] >= pivot[1]:
                hi.append(to_sort[i])
            else:
                lo.append(to_sort[i])
            #print lo
            i = i + 1
        return quicksort(hi) + [pivot] + quicksort(lo)

# Solution from Udacity video, only for sorting (NOT TESTED)
