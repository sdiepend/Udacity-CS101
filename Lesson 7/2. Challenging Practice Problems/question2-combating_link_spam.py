...
def detect_cycle(graph, base, startnode, k):
    cycles = {}
    if k == 0:
        ...
        for node in graph[startnode]:
            ...
            #print node
            if base == node:
                #print("B SN " + startnode)
                #print("N " + node)
                cycles[startnode] = [node]
                return cycles
                #graph[startnode].remove(node)
            else:
                return cycles
    else:
        for node in graph[startnode]:
            ...
            #print node
            #print cycles
            tempcycles = detect_cycle(graph, base, node, k-1)
            # if tempcycles != {}:
            #     if startnode in cycles:
            #         #print node
            #         cycles[startnode].append(node)
            #     else:
            #         #print("SN " + startnode)
            #         #print node
            #         cycles[startnode] = [node]

            cycles = dict(tempcycles, **cycles)

        return cycles

def remove_cycles(graph, k):
    cycles = {}
    for node in graph:
        cycles = dict(detect_cycle(graph, node, node, k), **cycles)

    for node in cycles:
        for e in cycles[node]:
            graph[node].remove(e)

    return graph 
    # return dict((k, v) for k, v in graph.iteritems() if v)
